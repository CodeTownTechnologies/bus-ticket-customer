...
import { Loader, Text12, Text14, CustomSafeArea, AppButton, Text10 } from '../../../../../components';
...
export default class RouteData extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({ selectedTrip: route.params, selectedTripIndex: route.params.selectedTripIndex  })
    }
        ...
        api.home.tripDetails(data).then(res => {
            ...
            if(res && res.data && res.data.status == "ok") {
                ...
                if(res.data.data && res.data.data.stoppage_points_list && res.data.data.stoppage_points_list.length > 0) {
                    res.data.data.stoppage_points_list.map(item => labels.push(item.title.substr(0, 40)))
                }
                this.setState({ tripData: res.data.data, labels, loading: false, stoppagePoints: res.data.data.stoppage_points_list })
            }
        })
    }
    ...
    render() {
        ...
        const {  trip_route_name, start, end, departure_location_latitude, departure_location_longitude } = this.state.selectedTrip
        ...
        return (
            <View style={styles.mainContainer}>
                ...
                <MapView
                    ...
                    style={{ flex: 1 }}
                    region={{
                        latitude: stoppagePoints && stoppagePoints.length > 0  ? parseFloat(stoppagePoints[0]['latitude']) : 37.78825,
                        longitude: stoppagePoints && stoppagePoints.length > 0 ? parseFloat(stoppagePoints[0]['longitude']) : -122.4324,
                        latitudeDelta: 1,
                ...
                >
                    ...
                    /> */}
                    {
                        stoppagePoints && stoppagePoints.length > 0 && stoppagePoints.map(place => {
                ...
                {this.renderHeader()}
                <View style={[styles.bottomContainer, routeDetailVisible ? { flex: 1 } : {}]}>
                    <View style={[AppStyles.shadow, styles.routeDetail]}>
                        ...
                        <View style={{ flex: 1 }}>
                            <Text14 title={trip_route_name} type="regular" addStyle={{ alignSelf: 'center'}}/>
                            {
                                !routeDetailVisible ? <Text12 title={start+" - "+ end} type="regular" addStyle={styles.startTime} />
                                    :
                                    <View style={styles.navView}>
                                        <TouchableOpacity style={styles.nextBtn} onPress={() => this.listNavBtnPressed('prev')} >
                                            <Image source={Images.arrowLeft} style={styles.navArrowImg} />
                                            <Text10 title="  Previous" type="light" addStyle={{ color: Colors.greyCb}}/>
                                        </TouchableOpacity>
                                        <Text12 title={start+" - "+ end} type="regular" addStyle={styles.startTime}/>
                                        <TouchableOpacity style={styles.nextBtn} onPress={() => this.listNavBtnPressed('next')} >
                                            <Text10 title="Next  " type="light" addStyle={{ color: Colors.greyCb }} />
                                            <Image source={Images.arrowRight} style={styles.navArrowImg}/>
                                        </TouchableOpacity>
                                    </View>
                            }
                        </View>
                    </View>
                    {
                        routeDetailVisible && labels.length > 0  &&
                            <View style={styles.stepContainer}>
                                ...
                                />
                                <TouchableOpacity style={[styles.closeBtn, AppStyles.shadow]} onPress={() => this.setState({ routeDetailVisible: false })}>
                                    <Image source={Images.crossRed} style={styles.closeIcon} resizeMode="contain"/>
                                </TouchableOpacity>
                            </View>
                    }
                    <AppButton type={"withoutContainer"} title={"Next"} buttonPressed={() => navigation.navigate('bookingDetail', this.state.selectedTrip)} containerStyle={styles.submitButton} />
                </View>
            </View>
        );
    }
