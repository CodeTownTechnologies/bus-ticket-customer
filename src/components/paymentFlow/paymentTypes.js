...
export default class paymentTypes extends Component {
    ...
    renderCreditCard(header, listData) {
        return (
            <View style={styles.card}>
                ...
                {
                    listData.map(item => (
                        <TouchableOpacity style={styles.cardList} onPress={() => this.props.selectPaymentType(item)}>
                            <Text14 type="light" title={item}/>
                            <Image source={(this.props.paymentType == item) ? Images.checkMark : Images.greenCheckBox} style={styles.greenCheckBox}/>
                        </TouchableOpacity>
                    ))
                }
            </View>
    ...
    render() {
        return (
            <View>
                <Text12 type="regular" title={"Pay with"} addStyle={styles.payWithText}/>
