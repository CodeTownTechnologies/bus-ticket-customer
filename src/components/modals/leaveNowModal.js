...
export default class LeaveNowModal extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedValue: 'Leave now',
      isButtonDisabled: true,
      // date: 'Select Date',
      //   time: 'Select Time',
      isDatePickerVisible: false,
      isTimeVisible: false,
      time: moment(new Date()).format('MM:HH a'),
      ...
      <>
        ...
        <TouchableOpacity
          ...
          <Text14
            type="regular"
            title="Done"
            addStyle={{alignSelf: 'center', color: Colors.green27}}
          />
        </TouchableOpacity>
      </>
    );
  ...
  renderCalender() {
    return (
      <Calendar
        ...
        onDayPress={(day) => {
          this.setState({date: day.dateString, isDatePickerVisible: false});
        }}
  ...
  renderTime() {
    const {isTimeVisible, selectedValue} = this.state;
    return (
      <View pointerEvents={selectedValue === 'Leave now' ? 'none' : 'auto'}>
        <DateTimePickerModal
          disabled={selectedValue === 'Leave now'}
